...
public class CategoryBudgetService {
    ...
    public CategoryBudget setOrUpdateCategoryBudget(Customer customer, Category category, BigDecimal amount) {
        ...
        Optional<TotalBudget> totalBudgetOpt = totalBudgetRepo.findByCustomerCustomerIDAndMonthAndYear(customerId,
                month, year);
        ...
        BigDecimal existingAmount = existingBudgetOpt.map(CategoryBudget::getMonthlyLimit).orElse(BigDecimal.ZERO);

        List<CategoryBudget> currentBudgets = categoryBudgetRepo.findByCustomerCustomerIDAndMonthAndYear(customerId,
                month, year);
        BigDecimal currentTotal = currentBudgets.stream()
                .map(CategoryBudget::getMonthlyLimit)
                .reduce(BigDecimal.ZERO, BigDecimal::add)
                .subtract(existingAmount); // remove current value if updating
